...
Задайте в Railway → Variables (локально — в окружении или `~/Desktop/tokens.txt`):

```env
WEB_APP_BASIC_AUTH_USERNAME="your_login"
WEB_APP_BASIC_AUTH_PASSWORD="your_strong_password"
```

При открытии сайта браузер запрашивает логин и пароль через HTTP Basic Auth.
Страница и API закрыты без авторизации; если переменные не настроены, сервис
возвращает 503 вместо открытого доступа. `/health` остаётся доступным для Railway.
Используйте публичный сервис только через HTTPS. Браузер может запоминать вход
на время сеанса; для проверки запроса пароля откройте приватное окно.

Ссылки на загруженные файлы подписываются на срок до семи дней, чтобы BytePlus
мог прочитать референсы без логина. Любой обладатель такой ссылки имеет доступ
к конкретному файлу до её истечения. Смена логина или пароля отзывает эти ссылки.
